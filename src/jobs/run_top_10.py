...
class RunTop10Job:
    ...
    def __init__(self, data_io: DataIOManager, logger: logging.Logger | None = None):
    ...
    def run(self) -> None:
        ...
        self._logger.info("Pedidos carregados: %d registros", df_pedidos.count())

        self._logger.info("Lendo dados de clientes (clientes_bronze)...")
        df_clientes = self._data_io.read("clientes_bronze")
        self._logger.info("Clientes carregados: %d registros", df_clientes.count())
